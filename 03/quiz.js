...
function print_imag(start_page){
    var str = '';
    var value = (start_page-1) * page_number;
    for(var i=value; i<value+3 && i != todayPhoto.length; i++){
        str += '<img src="'+todayPhoto[i].img+'"> ';
    }
    wrap.innerHTML = str;
    p_text.innerHTML = start_page + ' / ' + page_number;
}

print_imag(start_page);

document.getElementById('back').onclick=function(){
    if(start_page > 1)  print_imag(--start_page);
}
document.getElementById('next').onclick=function(){
    if(start_page < page_number)    print_imag(++start_page);
}